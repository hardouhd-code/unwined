...
async function updateCatalog() {
  ...
  const allWines = rawProducts
    .filter(p => {
      ...
      const isAccessory = ['bouchon','tire-bouchon','carafe','verre','stopper','dop','klem',
        'glas','glass','decant','tire bouchon','spuwemmer','folie','capsule','etiquette',
        'coffret accessoire','cave a vin','thermometre','verseur'].some(k => n.includes(k));
      const inStock = p.variants && p.variants.some(v => v.available);
      return inStock && !isAccessory;
    })
    .map(p => {
      // Tags Shopify = source la plus fiable pour la région (ex: ["Bordeaux", "Rouge", "2022"])
      const tagsStr = Array.isArray(p.tags) ? p.tags.join(' ') : (p.tags || '');
      const regionFromTags = inferRegion(tagsStr);
      const regionFromTitle = inferRegion(p.title);
      const country = inferCountry(p.title, p.vendor);
      const type = inferType(p.title, p.product_type);
      // Priorité : tags > titre > heuristique Château
      const smartRegion = regionFromTags || regionFromTitle || (
        p.title.toLowerCase().startsWith('château') && country === 'France' && type === 'rouge'
          ? 'Bordeaux' : ''
      );
      return {
        t: p.title,
