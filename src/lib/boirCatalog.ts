...
export function searchBoirLocal(query) {
  ...
  const results = [];
  const n = (s) => (s ? s.toString().toLowerCase() : "");

  for (let i = 0; i < BOIR_CATALOG.length; i++) {
    const wine = BOIR_CATALOG[i];
    let pts = 0;
    ...
    if (title.includes(term)) pts += 80;
    if (aop.includes(term)) pts += 65;
    if (region.includes(term)) pts += 50;
    if (country.includes(term)) pts += 40;
    if (type.includes(term)) pts += 30;
    if (vendor.includes(term)) pts += 20;

    if (pts > 0) {
      results.push({ w: wine, s: pts });
    }
  }
