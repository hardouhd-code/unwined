export async function callClaude(messages: any[], maxTokens = 600): Promise<string> {
  ...
  return d.content.map((b: any) => b.text || "").join("").replace(/```json|```/g, "").trim();
}

export function safeJson(txt: string, fallback: any = {}) {
...
export async function lookupWineOnline(wine: any) {
...
export async function askSommelier(history: any[], inventory: any[], userName: string) {
