...
const SmartScanner=({profile,onResult,onBack})=>{
  ...
  const scan=async(imageBase64)=>{
    setPhase("scanning");
    try{
      // Appel Claude Vision direct depuis le navigateur
      const CLAUDE_KEY="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"; // sk-ant-api03-...
      const body={
        model:"claude-sonnet-4-20250514",
        max_tokens:600,
        messages:[{role:"user",content:[
          {type:"image",source:{type:"base64",media_type:"image/jpeg",data:imageBase64}},
          {type:"text",text:`Expert en vins. Lis cette étiquette et réponds UNIQUEMENT en JSON valide :
{"name":"Nom complet","domaine":"Producteur","grape":"Cépage(s)","region":"Région, Pays","year":2020,"vivinoRating":4.1,"priceEst":"35 €","aeration":30,"pH":3.85,"tannins":7,"glycerol":7,"malo":true,"story":"2-3 phrases poétiques","emoji":"🍷","vibe":"cocon"}
...
Si illisible: {"error":"illisible"}`}
      ...
      console.log("Claude status:", r.status, rawText.slice(0,200));

      if(!r.ok) throw new Error(`HTTP ${r.status}: ${rawText.slice(0,150)}`);
      ...
      if(parsed.error==="illisible"){
        setErrMsg("Étiquette illisible. Essaie avec une meilleure lumière.");
      ...
      if(msg.includes("401")||msg.includes("auth")||msg.includes("API_KEY")){
        ...
        setErrMsg("Réponse illisible. Réessaie.");
      } else {
  ...
  const handleFileInput=(e)=>{
    ...
    reader.onload=(ev)=>{
      ...
      img.onload=()=>{
        ...
        // Max 800px et qualité 0.5 pour rester sous 500KB
        const maxW=800;
        const ratio=img.height/img.width;
        canvas.width=Math.min(img.width,maxW);
        canvas.height=Math.round(canvas.width*ratio);
        canvas.getContext("2d").drawImage(img,0,0,canvas.width,canvas.height);
        // Compresser progressivement jusqu'à < 400KB
        let quality=0.7;
        ...
        console.log("Photo finale:", Math.round(b64.length/1024)+"KB, qualité:"+quality.toFixed(1));
        scan(b64);
      };
  ...
  if(phase==="scanning"){
    return(
      <div style={{position:"fixed",inset:0,zIndex:600,background:"radial-gradient(ellipse at 50% 30%,#2a0808,#120303)",
        display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px"}}>
        <div style={{fontSize:52,marginBottom:24}}>🔍</div>
        <div style={{fontSize:18,fontFamily:"Playfair Display,serif",color:"#FAF8F0",marginBottom:8}}>
          Analyse en cours{".".repeat(dots)}
        </div>
        <div style={{fontSize:13,color:"rgba(245,245,220,.4)",fontFamily:"Cormorant Garamond,serif"}}>
      ...
      <div style={{position:"fixed",inset:0,zIndex:600,
        ...
        <div style={{padding:"48px 20px 48px"}}>
          <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:28}}>
            ...
            <span style={{fontSize:10,letterSpacing:3,color:"rgba(245,245,220,.3)",fontFamily:"Cormorant Garamond,serif",textTransform:"uppercase"}}>
              Analyse de l'étiquette
            </span>
          </div>
          <div style={{textAlign:"center",marginBottom:32}}>
            <div style={{fontSize:52,marginBottom:12}}>{wine.emoji||"🍷"}</div>
            <div style={{fontFamily:"Playfair Display,serif",fontSize:24,color:"#FAF8F0",marginBottom:4}}>{wine.name}</div>
            <div style={{fontSize:13,color:"rgba(245,245,220,.45)",fontFamily:"Cormorant Garamond,serif"}}>
              {wine.domaine} · {wine.region}
            </div>
          </div>
          <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:12,marginBottom:20}}>
            <div style={{background:"rgba(226,114,91,.12)",border:"1px solid rgba(226,114,91,.2)",borderRadius:16,padding:"16px 12px",textAlign:"center"}}>
              <div style={{fontSize:11,letterSpacing:2,color:"rgba(245,245,220,.4)",fontFamily:"Cormorant Garamond,serif",marginBottom:6,textTransform:"uppercase"}}>Vivino</div>
              <div style={{color:"#E2725B",fontSize:13,marginBottom:2}}>{"★".repeat(Math.round(wine.vivinoRating||4))}{"☆".repeat(5-Math.round(wine.vivinoRating||4))}</div>
              <div style={{fontSize:20,fontWeight:600,color:"#FAF8F0",fontFamily:"Playfair Display,serif"}}>{wine.vivinoRating||"—"}/5</div>
            </div>
            <div style={{background:"rgba(196,165,90,.08)",border:"1px solid rgba(196,165,90,.15)",borderRadius:16,padding:"16px 12px",textAlign:"center"}}>
              <div style={{fontSize:11,letterSpacing:2,color:"rgba(245,245,220,.4)",fontFamily:"Cormorant Garamond,serif",marginBottom:6,textTransform:"uppercase"}}>Prix moyen</div>
              <div style={{fontSize:20,fontWeight:600,color:"#C4A55A",fontFamily:"Playfair Display,serif"}}>€</div>
              <div style={{fontSize:16,color:"#FAF8F0",fontFamily:"Playfair Display,serif"}}>{wine.priceEst||"—"}</div>
            </div>
          </div>
          {wine.story&&<div style={{background:"rgba(245,245,220,.04)",border:"1px solid rgba(245,245,220,.08)",
            borderRadius:16,padding:"20px",marginBottom:20}}>
            <div style={{fontSize:11,letterSpacing:2,color:"rgba(245,245,220,.3)",fontFamily:"Cormorant Garamond,serif",marginBottom:10,textTransform:"uppercase"}}>Le récit du Sommelier</div>
            <div style={{fontSize:15,color:"rgba(245,245,220,.8)",fontFamily:"Cormorant Garamond,serif",lineHeight:1.7,fontStyle:"italic"}}>{wine.story}</div>
          </div>}
          {playlist&&<a href={playlist.url} target="_blank" rel="noopener noreferrer"
            style={{display:"flex",alignItems:"center",gap:12,background:"rgba(30,215,96,.08)",
              border:"1px solid rgba(30,215,96,.2)",borderRadius:14,padding:"14px 16px",
            ...
            <div>
              <div style={{fontSize:13,color:"rgba(245,245,220,.9)",fontFamily:"Playfair Display,serif"}}>{playlist.name}</div>
              <div style={{fontSize:11,color:"rgba(245,245,220,.4)",fontFamily:"Cormorant Garamond,serif"}}>{playlist.mood}</div>
            </div>
          </a>}
          <button onClick={()=>{
            if(onResult) onResult(wine);
          }} style={{width:"100%",padding:"16px",background:"linear-gradient(135deg,#E2725B,#c45a43)",
            border:"none",borderRadius:16,color:"#FAF8F0",fontSize:15,
            fontFamily:"Playfair Display,serif",marginBottom:12,cursor:"pointer"}}>
            + Ajouter à ma cave
          </button>
          <button onClick={()=>setPhase("camera")} style={{width:"100%",padding:"14px",
            background:"rgba(245,245,220,.05)",border:"1px solid rgba(245,245,220,.1)",
            borderRadius:16,color:"rgba(245,245,220,.6)",fontSize:13,
  ...
  // Phase camera — bouton photo native
  return(
    <div style={{position:"fixed",inset:0,zIndex:600,
      background:"radial-gradient(ellipse at 50% 20%,#1a0505,#120303)",
      display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
      padding:"32px 24px"}}>
      <button onClick={onBack} style={{position:"absolute",top:48,left:20,
        background:"rgba(245,245,220,.07)",border:"1px solid rgba(245,245,220,.12)",
        borderRadius:"50%",width:38,height:38,color:"rgba(245,245,220,.5)",fontSize:16}}>←</button>
      <div style={{fontSize:64,marginBottom:24}}>📸</div>
      <div style={{fontFamily:"Playfair Display,serif",fontSize:22,color:"#FAF8F0",
        textAlign:"center",marginBottom:8}}>Scanner une étiquette</div>
      <div style={{fontSize:14,color:"rgba(245,245,220,.45)",fontFamily:"Cormorant Garamond,serif",
        textAlign:"center",marginBottom:48,lineHeight:1.6}}>
        Prends une photo nette de l'étiquette.<br/>Bonne lumière, étiquette droite.
      </div>
      <label style={{
        display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",
        gap:12,width:"100%",maxWidth:320,padding:"24px",
        background:"linear-gradient(135deg,rgba(226,114,91,.2),rgba(196,165,90,.1))",
        border:"2px solid rgba(226,114,91,.4)",borderRadius:24,cursor:"pointer"}}>
        <input type="file" accept="image/*" capture="environment"
          onChange={handleFileInput}
          style={{display:"none"}} />
        <span style={{fontSize:36}}>📷</span>
        <span style={{fontFamily:"Playfair Display,serif",fontSize:16,color:"#FAF8F0"}}>
          Prendre une photo
        </span>
        <span style={{fontSize:12,color:"rgba(245,245,220,.4)",fontFamily:"Cormorant Garamond,serif"}}>
          Ouvre l'appareil photo natif
        </span>
      </label>
      <label style={{
        display:"flex",alignItems:"center",gap:10,marginTop:16,
        padding:"12px 24px",background:"rgba(245,245,220,.05)",
        border:"1px solid rgba(245,245,220,.1)",borderRadius:16,cursor:"pointer"}}>
        <input type="file" accept="image/*"
          onChange={handleFileInput}
          style={{display:"none"}} />
        <span style={{fontSize:18}}>🖼️</span>
        <span style={{fontSize:13,color:"rgba(245,245,220,.5)",fontFamily:"Cormorant Garamond,serif"}}>
