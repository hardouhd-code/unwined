...
const SmartScanner=({profile,onResult,onBack})=>{
  ...
  const scan=async()=>{
    ...
    // Appel IA : simule reconnaissance étiquette → sommelier
    ...
    const randBudget=budgets[Math.floor(Math.random()*budgets.length)];
    const result=await fetchVibeReco(randVibe,randStyle,randBudget,profile?.id||"velours-noir",Math.floor(Math.random()*5));
    setWine(result);
    setPhase("result");
  };
