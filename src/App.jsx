...
const supa = {
  ...
  async signUp(email, password) {
    ...
    supa._token = d.access_token; supa._user = d.user; return d;
  },
  ...
  saveSession(data) {
    localStorage.setItem("unwined_session", JSON.stringify({access_token:data.access_token, user:data.user}));
  },
          ...
          <div style={{background:C.bgCard,border:"1px solid rgba(139,90,60,.12)",borderRadius:18,padding:"16px 20px",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <span style={{fontSize:13,color:C.muted,fontFamily:"Cormorant Garamond,serif"}}>bouteille{(wine.quantity||0)>1?"s":""}</span>
              </div>
            </div>
            {onDelete&&<button onClick={()=>{if(window.confirm("Supprimer ce vin ?"))onDelete(wine.id);}} style={{background:"none",border:"1px solid rgba(200,80,58,.25)",borderRadius:20,padding:"8px 14px",color:C.terra,fontSize:13,fontFamily:"Cormorant Garamond,serif",whiteSpace:"nowrap",flexShrink:0}}>🗑 Supprimer</button>}
          </div>
...
const AuthScreen=({onAuth})=>{
  ...
  const handle=async()=>{
    ...
    try{
      ...
      if(mode==="signup"){
        ...
        if(!data.access_token){setSuccess("Vérifiez votre email pour confirmer votre compte.");setLoading(false);return;}
      } else {
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px",fontFamily:"Cormorant Garamond,serif"}}>
      {/* Logo */}
      <div style={{textAlign:"center",marginBottom:40}}>
        <div style={{fontSize:9,color:C.muted,letterSpacing:".38em",textTransform:"uppercase",marginBottom:10}}>Votre sommelier personnel</div>
        <div style={{display:"inline-flex",alignItems:"center",gap:10}}>
          <span style={{fontSize:22,filter:`drop-shadow(0 3px 8px rgba(200,80,58,.3))`}}>🍷</span>
      ...
      <div style={{width:"100%",maxWidth:360,background:C.bgCard,borderRadius:24,padding:"28px 24px",boxShadow:"0 8px 32px rgba(139,90,60,.1)"}}>
        ...
        <div style={{marginBottom:12}}>
          <input value={email} onChange={e=>setEmail(e.target.value)} type="email" placeholder="Email" style={{width:"100%",background:"rgba(139,90,60,.05)",border:"1px solid rgba(139,90,60,.18)",borderRadius:12,padding:"13px 15px",color:C.cream,fontSize:15,fontFamily:"Cormorant Garamond,serif",boxSizing:"border-box"}}/>
        </div>
        <div style={{marginBottom:20}}>
          <input value={password} onChange={e=>setPassword(e.target.value)} onKeyDown={e=>e.key==="Enter"&&handle()} type="password" placeholder="Mot de passe" style={{width:"100%",background:"rgba(139,90,60,.05)",border:"1px solid rgba(139,90,60,.18)",borderRadius:12,padding:"13px 15px",color:C.cream,fontSize:15,fontFamily:"Cormorant Garamond,serif",boxSizing:"border-box"}}/>
        </div>
...
export default function UnwinedApp() {
  ...
  const loadUserData=async(userId)=>{
    ...
    if(profile){
      ...
      if(profile.cave) { try{ setDb(JSON.parse(profile.cave)); }catch{} }
    } else {
  ...
  if(authLoading) return(
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:C.bg,display:"flex",alignItems:"center",justifyContent:"center"}}>
      <div style={{textAlign:"center"}}><div style={{fontSize:48,marginBottom:16,animation:"pulse 1.5s ease-in-out infinite"}}>🍷</div><div style={{fontSize:14,color:C.muted,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic"}}>Chargement…</div></div>
    </div>
  );
  ...
  return(
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:C.bg,fontFamily:"Cormorant Garamond,Georgia,serif",display:"flex",flexDirection:"column",position:"relative"}}>
      ...
      {/* ── HEADER ── */}
      {!["detail","scan","add"].includes(screen)&&(
          ...
          <div style={{textAlign:"center",marginBottom:16}}>
            <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:8}}>
              <div style={{fontSize:11,color:C.muted,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic"}}>
                {syncing?"⟳ sync…":"✓ "+( user?.email?.split("@")[0]||"")}
              </div>
