...
function safeJson(txt, fallback={}) {
  ...
  // Tentative de nettoyage plus agressive
  try {
...
{"description":"2-3 phrases style/arômes/structure","grapes":"cépages","serving":"température et accord mets","price_range":"fourchette prix ex: 15-25€","peak":"fenêtre dégustation","score":"note critique si connue","vivino_query":"termes recherche Vivino"}`}], 500);
  return safeJson(txt, {});
}
...
const Tag=({children,color})=>(
  <span style={{fontSize:11,color:color||C.accent,background:color?`${color}18`:"rgba(139,90,60,.1)",border:`1px solid ${color||C.accent}33`,borderRadius:20,padding:"4px 11px",fontFamily:"Cormorant Garamond,serif",lineHeight:1}}>{children}</span>
);
...
const WineInfoCard=({wine})=>{
  ...
  if(!info&&!loading)return(
    <button onClick={fetch_info} style={{width:"100%",background:"rgba(184,134,42,.08)",border:`1px solid rgba(184,134,42,.25)`,borderRadius:14,padding:"13px 16px",display:"flex",alignItems:"center",gap:12,marginBottom:14,transition:"all .2s"}}>
      ...
      <div style={{textAlign:"left"}}>
        <div style={{fontSize:12,color:C.gold,fontFamily:"Playfair Display,serif",fontWeight:500}}>Fiche en ligne</div>
        <div style={{fontSize:11,color:C.muted,fontFamily:"Cormorant Garamond,serif",marginTop:2}}>Prix, accord, apogée, note critique…</div>
      </div>
  ...
  if(loading)return(
    <div style={{background:"rgba(184,134,42,.06)",border:`1px solid rgba(184,134,42,.18)`,borderRadius:14,padding:"16px",marginBottom:14,textAlign:"center"}}>
      ...
      <div style={{fontSize:12,color:C.muted,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic"}}>Consultation des bases de données…</div>
    </div>
  );
  if(error)return(
    <div style={{background:"rgba(200,80,58,.06)",border:`1px solid rgba(200,80,58,.2)`,borderRadius:14,padding:"12px 16px",marginBottom:14}}>
      <span style={{fontSize:11,color:C.terra,fontFamily:"Cormorant Garamond,serif"}}>Impossible de récupérer les infos. </span>
      <button onClick={fetch_info} style={{color:C.gold,textDecoration:"underline",fontSize:11,fontFamily:"Cormorant Garamond,serif"}}>Réessayer</button>
    </div>
  );
  return(
    <div style={{background:"rgba(184,134,42,.07)",border:`1px solid rgba(184,134,42,.22)`,borderRadius:16,padding:"16px",marginBottom:14,animation:"fadeIn .4s ease"}}>
      <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
        <div style={{fontSize:9,color:C.gold,letterSpacing:".2em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif"}}>🌐 Fiche en ligne</div>
        {info.score&&<div style={{fontSize:13,color:C.gold,fontFamily:"Playfair Display,serif",fontWeight:600}}>{info.score}</div>}
      </div>
      {info.description&&<p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",lineHeight:1.8,marginBottom:12}}>{info.description}</p>}
      <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8,marginBottom:12}}>
        {info.grapes&&<div style={{background:"rgba(139,90,60,.06)",borderRadius:10,padding:"8px 10px"}}><div style={{fontSize:8,color:C.muted,letterSpacing:".15em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",marginBottom:3}}>Cépages</div><div style={{fontSize:12,color:C.subtext,fontFamily:"Cormorant Garamond,serif"}}>{info.grapes}</div></div>}
        {info.price_range&&<div style={{background:"rgba(139,90,60,.06)",borderRadius:10,padding:"8px 10px"}}><div style={{fontSize:8,color:C.muted,letterSpacing:".15em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",marginBottom:3}}>Prix</div><div style={{fontSize:13,color:C.gold,fontFamily:"Playfair Display,serif",fontWeight:600}}>{info.price_range}</div></div>}
        {info.serving&&<div style={{background:"rgba(139,90,60,.06)",borderRadius:10,padding:"8px 10px"}}><div style={{fontSize:8,color:C.muted,letterSpacing:".15em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",marginBottom:3}}>Service</div><div style={{fontSize:11,color:C.subtext,fontFamily:"Cormorant Garamond,serif"}}>{info.serving}</div></div>}
        {info.peak&&<div style={{background:"rgba(139,90,60,.06)",borderRadius:10,padding:"8px 10px"}}><div style={{fontSize:8,color:C.muted,letterSpacing:".15em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",marginBottom:3}}>Apogée</div><div style={{fontSize:11,color:C.subtext,fontFamily:"Cormorant Garamond,serif"}}>{info.peak}</div></div>}
      </div>
      <a href={`https://www.vivino.com/search/wines?q=${encodeURIComponent(info.vivino_query||q)}`} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,background:`rgba(200,80,58,.12)`,border:`1px solid rgba(200,80,58,.3)`,borderRadius:20,padding:"7px 16px",textDecoration:"none",color:C.terra,fontSize:10,letterSpacing:".12em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif"}}>🔍 Voir sur Vivino</a>
    </div>
  );
};
...
const AddWineForm=({onSave,onCancel})=>{
  ...
  const lbl={fontSize:9,color:C.accent,letterSpacing:".25em",textTransform:"uppercase",marginBottom:8,fontFamily:"Cormorant Garamond,serif",display:"block"};
    ...
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column"}}>
      <div style={{padding:"36px 20px 0",flexShrink:0}}>
        <div style={{display:"flex",alignItems:"center",gap:12,marginBottom:20}}>
          ...
          <div>
            <div style={{fontSize:9,color:C.muted,letterSpacing:".3em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif"}}>Étape {step} / {totalSteps}</div>
      ...
      <div style={{flex:1,overflowY:"auto",padding:"0 20px 120px"}}>
        {step===1&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            ...
            <p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginBottom:28}}>Commençons par le commencement.</p>
            <div style={{display:"flex",flexDirection:"column",gap:10}}>
              {WINE_TYPES.map((t,i)=>(
                <button key={t.id} onClick={()=>{haptic(30);up("type",t.id);}} style={{background:form.type===t.id?typeLight(t.id):"rgba(139,90,60,.05)",border:`1px solid ${form.type===t.id?t.color+"66":"rgba(139,90,60,.15)"}`,borderRadius:16,padding:"16px 18px",display:"flex",alignItems:"center",gap:14,animation:`fadeUp .3s ease ${i*.06}s both`,opacity:0,transition:"all .2s"}}>
                  ...
                  {form.type===t.id&&<div style={{marginLeft:"auto",width:20,height:20,borderRadius:"50%",background:t.color,display:"flex",alignItems:"center",justifyContent:"center",fontSize:11,color:"#fff",fontWeight:700}}>✓</div>}
                </button>
              ))}
            </div>
          </div>
        ...
        {step===2&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            ...
            <p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginBottom:28}}>Le terroir raconte toujours une histoire.</p>
            <div style={{display:"grid",gridTemplateColumns:"1fr 1fr",gap:8}}>
              {Object.entries(COUNTRIES).map(([country,data])=>(
                <button key={country} onClick={()=>{haptic(20);up("country",country);up("region","");}} style={{background:form.country===country?"rgba(184,134,42,.15)":"rgba(139,90,60,.05)",border:`1px solid ${form.country===country?"rgba(184,134,42,.5)":"rgba(139,90,60,.15)"}`,borderRadius:12,padding:"11px 10px",display:"flex",alignItems:"center",gap:8,transition:"all .2s"}}>
                  ...
                  <span style={{fontSize:12,color:form.country===country?C.gold:C.subtext,fontFamily:"Cormorant Garamond,serif",fontWeight:form.country===country?600:400}}>{country}</span>
        ...
        {step===4&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            ...
            <p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginBottom:28}}>Plus vous en dites, mieux je vous connais.</p>
        ...
        {step===5&&(
          <div style={{animation:"fadeUp .4s ease"}}>
            ...
            <p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginBottom:28}}>Votre mémoire gustative est précieuse.</p>
            <div style={{display:"flex",gap:12,marginBottom:24}}>
              {[{val:true,label:"Oui, je l'ai bu",emoji:"✓",c:C.sauge},{val:false,label:"Non, pas encore",emoji:"○",c:C.gold}].map(opt=>(
                <button key={String(opt.val)} onClick={()=>{haptic(40);up("tasted",opt.val);up("storage",!opt.val);}} style={{flex:1,padding:"18px 10px",background:form.tasted===opt.val?`${opt.c}18`:"rgba(139,90,60,.05)",border:`1px solid ${form.tasted===opt.val?opt.c+"55":"rgba(139,90,60,.15)"}`,borderRadius:16,display:"flex",flexDirection:"column",alignItems:"center",gap:10,transition:"all .25s"}}>
                  ...
                  <span style={{fontSize:13,color:form.tasted===opt.val?C.cream:C.subtext,fontFamily:"Cormorant Garamond,serif",textAlign:"center",lineHeight:1.3}}>{opt.label}</span>
                </button>
              ))}
            </div>
            {form.tasted===true&&(
              <div style={{background:"rgba(200,80,58,.07)",border:`1px solid rgba(200,80,58,.2)`,borderRadius:16,padding:"20px 16px",marginBottom:14,animation:"fadeUp .3s ease"}}>
                ...
                <div style={{fontSize:11,color:C.muted,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",textAlign:"center",marginBottom:16}}>0 = à éviter · 5 = expérience absolue</div>
            ...
            {form.tasted===false&&(
              <div style={{background:"rgba(184,134,42,.08)",border:`1px solid rgba(184,134,42,.22)`,borderRadius:16,padding:"16px",animation:"fadeUp .3s ease"}}>
                <div style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",lineHeight:1.7,textAlign:"center"}}>Ce vin sera stocké dans votre cave.<br/><span style={{color:C.gold}}>Vous pourrez l'évaluer plus tard.</span></div>
              </div>
            )}
          </div>
        )}
      ...
      <div style={{position:"fixed",bottom:0,left:"50%",transform:"translateX(-50%)",width:"100%",maxWidth:430,padding:"12px 20px 32px",background:`linear-gradient(transparent,${C.bg} 35%)`}}>
        {step<5?(
          <button onClick={()=>{if(canNext()){haptic(40);setStep(s=>s+1);}}} style={{width:"100%",padding:"16px",background:canNext()?`linear-gradient(135deg,${C.terra},${C.terraD})`:"rgba(139,90,60,.12)",color:canNext()?"#fff":"rgba(61,43,26,.3)",border:"none",borderRadius:50,fontSize:12,letterSpacing:".2em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",fontWeight:700,boxShadow:canNext()?`0 8px 28px rgba(200,80,58,.3)`:"none",transition:"all .3s"}}>Continuer →</button>
        ):(
          <button onClick={handleSave} disabled={form.tasted===null||(form.tasted===true&&form.rating===null)} style={{width:"100%",padding:"16px",background:(form.tasted!==null&&(form.tasted===false||form.rating!==null))?`linear-gradient(135deg,${C.sauge},#4a7a2e)`:"rgba(139,90,60,.12)",color:(form.tasted!==null&&(form.tasted===false||form.rating!==null))?"#fff":"rgba(61,43,26,.3)",border:"none",borderRadius:50,fontSize:12,letterSpacing:".2em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",fontWeight:700,transition:"all .3s"}}>✓ Ajouter à ma cave</button>
        )}
      </div>
    </div>
  );
    ...
    <div style={{minHeight:"100vh",background:C.bg,overflowY:"auto"}}>
      <div style={{background:`linear-gradient(160deg,${tl} 0%,transparent 40%)`,minHeight:"100vh"}}>
        <div style={{padding:"36px 20px 0",display:"flex",alignItems:"center",gap:12,marginBottom:24}}>
          ...
          <div>
            <div style={{fontSize:9,color:C.muted,letterSpacing:".25em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif"}}>{typeEmoji(wine.type)} {WINE_TYPES.find(t=>t.id===wine.type)?.label}</div>
        ...
        <div style={{padding:"0 20px 80px"}}>
          <div style={{background:C.bgCard,border:`1px solid ${tc}33`,borderRadius:22,padding:"22px",marginBottom:14,boxShadow:`0 4px 24px rgba(139,90,60,.1)`}}>
            ...
            <div style={{display:"flex",gap:8,justifyContent:"center",flexWrap:"wrap"}}>
              {[wine.region&&`📍 ${wine.region}`,wine.country&&`${COUNTRIES[wine.country]?.flag||"🌍"} ${wine.country}`,wine.year&&`📅 ${wine.year}`].filter(Boolean).map(tag=>(
                <span key={tag} style={{fontSize:12,color:C.subtext,background:"rgba(139,90,60,.07)",border:"1px solid rgba(139,90,60,.15)",borderRadius:20,padding:"5px 12px",fontFamily:"Cormorant Garamond,serif"}}>{tag}</span>
              ))}
            </div>
          </div>
          ...
          <div style={{background:C.bgCard,border:C.border,borderRadius:18,padding:"18px",marginBottom:14,boxShadow:`0 2px 12px rgba(139,90,60,.06)`}}>
            {wine.tasted&&rating!==null?(<div style={{textAlign:"center"}}><div style={{fontSize:10,color:C.gold,letterSpacing:".22em",textTransform:"uppercase",marginBottom:10,fontFamily:"Cormorant Garamond,serif"}}>Votre note</div><div style={{fontSize:44,fontFamily:"Playfair Display,serif",color:tc,fontWeight:600,lineHeight:1}}>{rating}<span style={{fontSize:18,color:C.muted}}>/5</span></div><div style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginTop:6}}>{["À éviter 🚫","Décevant 😕","Correct 😐","Bien 🙂","Très bon ⭐","Exceptionnel ✨"][rating]}</div><button onClick={()=>setShowRate(true)} style={{marginTop:10,background:"none",color:"rgba(184,134,42,.6)",fontSize:10,fontFamily:"Cormorant Garamond,serif",letterSpacing:".1em",textDecoration:"underline"}}>Modifier ma note</button></div>
            ...
            ):(<div style={{textAlign:"center"}}><div style={{fontSize:30,marginBottom:8}}>🗄️</div><div style={{fontSize:14,color:C.cream,fontFamily:"Playfair Display,serif",marginBottom:6}}>En cave</div><div style={{fontSize:12,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginBottom:14}}>Stocké pour plus tard</div><button onClick={()=>setShowRate(true)} style={{background:`linear-gradient(135deg,${C.terra},${C.terraD})`,color:"#fff",border:"none",padding:"10px 22px",borderRadius:50,fontSize:11,letterSpacing:".15em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif"}}>Je l'ai goûté !</button></div>)}
          </div>

          <WineInfoCard wine={wine}/>

          {wine.story&&(<div style={{background:"rgba(200,80,58,.06)",borderLeft:`3px solid ${C.terra}`,borderRadius:"0 14px 14px 0",padding:"14px 16px",marginBottom:14}}><div style={{fontSize:9,color:C.terra,letterSpacing:".22em",textTransform:"uppercase",marginBottom:8,fontFamily:"Cormorant Garamond,serif"}}>Le Récit du Terroir</div><p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",lineHeight:1.85}}>{wine.story}</p></div>)}
          {wine.anecdote&&(<div style={{background:"rgba(184,134,42,.07)",border:`1px solid rgba(184,134,42,.2)`,borderRadius:14,padding:"13px 15px",marginBottom:14}}><div style={{fontSize:9,color:C.gold,letterSpacing:".18em",textTransform:"uppercase",marginBottom:6,fontFamily:"Cormorant Garamond,serif"}}>✦ Le saviez-vous ?</div><p style={{fontSize:12,color:C.subtext,fontFamily:"Cormorant Garamond,serif",lineHeight:1.75}}>{wine.anecdote}</p></div>)}
          {wine.notes&&(<div style={{background:C.bgCard,border:C.border,borderRadius:14,padding:"13px 15px"}}><div style={{fontSize:9,color:C.muted,letterSpacing:".18em",textTransform:"uppercase",marginBottom:6,fontFamily:"Cormorant Garamond,serif"}}>Ma note personnelle</div><p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",lineHeight:1.75}}>{wine.notes}</p></div>)}
        </div>
      </div>
...
{"topPick":{"name":"","producer":"","type":"rouge|blanc|rose|mousseux|autre","country":"","region":"","year":2020,"match":85,"why":"2 phrases poétiques","story":"3 phrases terroir","emoji":"🍷","grapes":"","price_range":""}}`;
  ...
  if(phase==="scanning")return(<div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:22,padding:32}}><div style={{position:"relative",width:72,height:72}}><div style={{position:"absolute",inset:0,borderRadius:"50%",border:`2px solid rgba(184,134,42,.2)`}}/><div style={{position:"absolute",inset:0,borderRadius:"50%",border:"2px solid transparent",borderTopColor:C.gold,animation:"spin 1.2s linear infinite"}}/><div style={{position:"absolute",inset:0,display:"flex",alignItems:"center",justifyContent:"center",fontSize:26}}>🔍</div></div><div style={{textAlign:"center"}}><div style={{fontSize:18,fontFamily:"Playfair Display,serif",color:C.cream,marginBottom:6}}>Analyse en cours…</div><div style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic"}}>Le Sommelier lit l'étiquette</div></div></div>);

  if(phase==="error")return(<div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",gap:18,padding:32,textAlign:"center"}}><div style={{fontSize:48}}>⚠️</div><div style={{fontSize:15,fontFamily:"Playfair Display,serif",color:C.cream}}>{errMsg}</div><button onClick={()=>setPhase("idle")} style={{background:`rgba(200,80,58,.12)`,border:`1px solid rgba(200,80,58,.35)`,borderRadius:22,padding:"11px 26px",color:C.terra,fontSize:13,fontFamily:"Playfair Display,serif"}}>Réessayer</button><button onClick={onBack} style={{background:"none",color:C.muted,fontSize:12,fontFamily:"Cormorant Garamond,serif"}}>← Retour</button></div>);
      ...
      <div style={{minHeight:"100vh",background:C.bg,overflowY:"auto"}}>
        <div style={{background:`linear-gradient(160deg,${typeLight(p.type)} 0%,transparent 40%)`}}>
          <div style={{padding:"36px 20px 80px"}}>
            ...
            <div style={{background:C.bgCard,border:`1px solid ${tc}33`,borderRadius:22,padding:"22px",marginBottom:14,boxShadow:`0 4px 24px rgba(139,90,60,.1)`}}>
              <div style={{fontSize:9,color:C.gold,letterSpacing:".3em",textTransform:"uppercase",marginBottom:12,fontFamily:"Cormorant Garamond,serif",textAlign:"center"}}>Recommandation du Sommelier</div>
              ...
              <div style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",textAlign:"center",marginBottom:14}}>{p.producer}</div>
              <div style={{background:"rgba(139,90,60,.06)",borderRadius:14,padding:"12px",textAlign:"center",marginBottom:14}}>
                ...
                <div style={{fontSize:9,color:C.muted,letterSpacing:".15em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",marginTop:2}}>match avec vos goûts</div>
              ...
              <div style={{display:"flex",gap:7,justifyContent:"center",flexWrap:"wrap",marginBottom:12}}>
                {[WINE_TYPES.find(t=>t.id===p.type)?.label,p.region,p.country,p.year&&String(p.year)].filter(Boolean).map(tag=>(
                  <span key={tag} style={{fontSize:11,color:C.subtext,background:"rgba(139,90,60,.07)",border:"1px solid rgba(139,90,60,.15)",borderRadius:20,padding:"4px 11px",fontFamily:"Cormorant Garamond,serif"}}>{tag}</span>
                ))}
              </div>
              {p.grapes&&<div style={{fontSize:12,color:C.muted,fontFamily:"Cormorant Garamond,serif",textAlign:"center",marginBottom:8}}>🍇 {p.grapes}</div>}
              {p.price_range&&<div style={{fontSize:13,color:C.gold,fontFamily:"Playfair Display,serif",textAlign:"center",marginBottom:10,fontWeight:600}}>{p.price_range}</div>}
            ...
            {p.story&&<div style={{background:"rgba(200,80,58,.06)",borderLeft:`3px solid ${C.terra}`,borderRadius:"0 14px 14px 0",padding:"14px 16px",marginBottom:14}}><div style={{fontSize:9,color:C.terra,letterSpacing:".2em",textTransform:"uppercase",marginBottom:8,fontFamily:"Cormorant Garamond,serif"}}>Le Récit</div><p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",lineHeight:1.85}}>{p.story}</p></div>}
            <button onClick={()=>{haptic(60);onResult({id:Date.now(),...wineObj,tasted:false,storage:true,rating:null,notes:"",addedAt:new Date().toLocaleDateString("fr-FR")});}} style={{width:"100%",background:`linear-gradient(135deg,${C.terra},${C.terraD})`,color:"#fff",border:"none",padding:"16px",borderRadius:50,fontSize:12,letterSpacing:".2em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",fontWeight:700,marginBottom:10,boxShadow:`0 8px 28px rgba(200,80,58,.25)`}}>+ Ajouter à ma cave</button>
            <button onClick={()=>setPhase("idle")} style={{width:"100%",background:"rgba(139,90,60,.08)",border:`1px solid rgba(139,90,60,.2)`,color:C.muted,padding:"12px",borderRadius:50,fontSize:11,letterSpacing:".15em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif"}}>↺ Scanner une autre</button>
          </div>
        </div>
      </div>
    );
  ...
  return(
    <div style={{minHeight:"100vh",background:C.bg,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"32px 24px"}}>
      ...
      <label style={{width:"100%",maxWidth:320,display:"flex",flexDirection:"column",alignItems:"center",gap:12,padding:"26px 22px",background:`rgba(200,80,58,.08)`,border:`2px solid rgba(200,80,58,.3)`,borderRadius:22,cursor:"pointer"}}>
        ...
        <span style={{fontSize:11,color:C.muted,fontFamily:"Cormorant Garamond,serif"}}>Ouvre l'appareil photo</span>
      </label>
      <label style={{display:"flex",alignItems:"center",gap:10,marginTop:12,padding:"11px 20px",background:"rgba(139,90,60,.06)",border:`1px solid rgba(139,90,60,.18)`,borderRadius:16,cursor:"pointer"}}>
        ...
        <span style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif"}}>Choisir depuis la galerie</span>
      </label>
    </div>
  );
};
...
const MaCave=({db,onOpenWine,onAdd})=>{
  ...
  if(db.length===0)return(
    <div style={{flex:1,display:"flex",flexDirection:"column",alignItems:"center",justifyContent:"center",padding:"40px 24px",textAlign:"center"}}>
      ...
      <button onClick={onAdd} style={{background:`linear-gradient(135deg,${C.terra},${C.terraD})`,color:"#fff",border:"none",padding:"15px 30px",borderRadius:50,fontSize:12,letterSpacing:".2em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",fontWeight:600,boxShadow:`0 8px 24px rgba(200,80,58,.25)`}}>+ Ajouter mon premier vin</button>
    </div>
  );
      ...
      <div style={{display:"grid",gridTemplateColumns:"repeat(4,1fr)",gap:8,marginBottom:14}}>
        {[[stats.total,"Vins",C.cream],[stats.inCave,"En cave",C.gold],[stats.tasted,"Goûtés",C.terra],[stats.avg,"Moy.",C.sauge]].map(([v,l,col])=>(
          <div key={l} style={{background:C.bgCard,border:C.border,borderRadius:14,padding:"11px 7px",textAlign:"center",boxShadow:`0 2px 8px rgba(139,90,60,.08)`}}>
            ...
            <div style={{fontSize:8,color:C.muted,letterSpacing:".1em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",marginTop:2}}>{l}</div>
          </div>
        ))}
      </div>
          ...
          {displayed.map((wine,i)=>{const tc=typeColor(wine.type);const tl=typeLight(wine.type);return(
            <button key={wine.id} onClick={()=>{haptic(30);onOpenWine(wine);}} style={{background:C.bgCard,border:`1px solid ${tc}22`,borderRadius:16,padding:"14px",textAlign:"left",transition:"all .2s",animation:`fadeUp .35s ease ${i*.04}s both`,opacity:0,display:"flex",alignItems:"center",gap:12,boxShadow:`0 2px 10px rgba(139,90,60,.07)`}}>
              ...
              <div style={{flex:1,minWidth:0}}>
                ...
                <div style={{fontSize:11,color:C.subtext,fontFamily:"Cormorant Garamond,serif",marginTop:2,overflow:"hidden",textOverflow:"ellipsis",whiteSpace:"nowrap"}}>{[wine.region,wine.country,wine.year].filter(Boolean).join(" · ")}</div>
              </div>
              <div style={{flexShrink:0,textAlign:"right"}}>
                {wine.tasted&&wine.rating!==null?(<div><div style={{fontSize:17,fontFamily:"Playfair Display,serif",color:wine.rating>=4?C.sauge:wine.rating>=2?C.gold:C.terra,fontWeight:600}}>{wine.rating}/5</div><div style={{fontSize:9,color:C.muted,fontFamily:"Cormorant Garamond,serif"}}>{"★".repeat(wine.rating)}{"☆".repeat(5-wine.rating)}</div></div>):!wine.tasted?(<div style={{fontSize:9,color:C.gold,background:"rgba(184,134,42,.1)",border:"1px solid rgba(184,134,42,.25)",borderRadius:10,padding:"4px 8px",fontFamily:"Cormorant Garamond,serif"}}>🗄️ Cave</div>):(<div style={{fontSize:9,color:C.muted,fontFamily:"Cormorant Garamond,serif"}}>Non noté</div>)}
              </div>
            </button>
          );})}
        </div>
...
/* ═══════════════════════════════════════════════════════════════
   HELPER — Vivino direct (api/vivino → vivino.com/api/explore)
   Sans Apify, sans token, résultats en ~1-2s
═══════════════════════════════════════════════════════════════ */

// Appel direct au proxy Vivino
async function searchVivino(query, opts = {}) {
  const res = await fetch("/api/vivino", {
    method: "POST",
    headers: { "Content-Type": "application/json" },
    body: JSON.stringify({ query, country: "FR", minRating: 3.5, limit: 6, ...opts }),
  });
  if (!res.ok) {
    const err = await res.json().catch(() => ({}));
    throw new Error(err.error || `Vivino HTTP ${res.status}`);
  }
  const data = await res.json();
  return data.wines || [];
}

// Générer des requêtes de recherche courtes pour Vivino via Claude
async function generateVivinoQueries(db) {
  const liked    = db.filter(w=>w.tasted&&w.rating>=3).map(w=>`${w.type} ${w.region}`).join(", ");
  const disliked = db.filter(w=>w.tasted&&w.rating<=1).map(w=>w.type).join(", ");
  const prompt = `Tu es sommelier. Génère 3 requêtes de recherche courtes pour Vivino (API explore).${liked?` L'utilisateur aime : ${liked}.`:""}${disliked?` Éviter : ${disliked}.`:""}
Requêtes variées, courtes (2-4 mots), en anglais, différents pays/types.
UNIQUEMENT JSON sans markdown : {"queries":["malbec mendoza","burgundy pinot noir","white rhone viognier"]}`;
  const txt = await callClaude([{role:"user",content:prompt}], 200);
  const parsed = safeJson(txt, {});
  return parsed.queries || ["bordeaux red","burgundy pinot noir","rhone viognier"];
}
/* ═══════════════════════════════════════════════════════════════
...
const Decouvrir=({db})=>{
  ...
  const loadVivinoSuggestions=async()=>{
    ...
    setVivinoMsg("Le Sommelier prépare votre sélection…");
    try {
      // Étape 1 : Claude génère 3 requêtes de recherche
      const queries = await generateVivinoQueries(db);
      setVivinoPhase("fetching");
      setVivinoMsg("Consultation Vivino en direct…");
      // Étape 2 : Recherches parallèles sur Vivino (~1-2s chacune)
      const results = await Promise.allSettled(queries.map(q => searchVivino(q)));
      const all = results.flatMap(r => r.status==="fulfilled" ? r.value : []);
      // Dédupliquer par wine_id ou nom
      const seen = new Set();
      const wines = all.filter(w => {
        const key = w.wine_id || w.name;
  ...
  const doSearch=async()=>{
    ...
    try {
      // Étape 1 : Claude optimise la requête pour Vivino
      const txt = await callClaude([{role:"user",content:`Tu es sommelier. Traduis cette recherche en 2 requêtes courtes pour Vivino (en anglais, 2-4 mots max).
Recherche : "${query}"
UNIQUEMENT JSON : {"queries":["cabernet sauvignon napa","merlot pomerol"]}`}], 150);
      const parsed = safeJson(txt, {});
      const queries = parsed.queries || [query];
      setSearchPhase("vivino");
      // Étape 2 : Recherche directe Vivino
      const results = await Promise.allSettled(queries.map(q => searchVivino(q)));
      const all = results.flatMap(r => r.status==="fulfilled" ? r.value : []);
      const seen = new Set();
      const wines = all.filter(w => {
      ...
      <div style={{background:C.bgCard,border:`1px solid ${tc}28`,borderRadius:18,padding:"16px",animation:`fadeUp .4s ease ${idx*.07}s both`,opacity:0,boxShadow:`0 3px 14px rgba(139,90,60,.07)`}}>
        <div style={{display:"flex",gap:12,marginBottom:12}}>
          ...
          <div style={{flex:1,minWidth:0}}>
            ...
            <div style={{fontSize:11,color:C.subtext,fontFamily:"Cormorant Garamond,serif"}}>{[wine.producer,wine.year].filter(Boolean).join(" · ")}</div>
          </div>
          {/* Note Vivino */}
          {wine.rating&&(
            <div style={{flexShrink:0,textAlign:"right"}}>
              ...
              <div style={{fontSize:8,color:C.muted,fontFamily:"Cormorant Garamond,serif",letterSpacing:".06em"}}>/ 5.0</div>
              {wine.ratings_count&&<div style={{fontSize:9,color:C.muted,fontFamily:"Cormorant Garamond,serif"}}>{wine.ratings_count.toLocaleString()} avis</div>}
            </div>
          )}
        </div>
        ...
        {/* Description */}
        {wine.description&&(
          <p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",lineHeight:1.75,marginBottom:10}}>{wine.description.slice(0,200)}{wine.description.length>200?"…":""}</p>
        )}

        {/* Prix + profil gustatif */}
        <div style={{display:"flex",gap:8,flexWrap:"wrap",marginBottom:12,alignItems:"center"}}>
          {wine.price_range&&(
            <div style={{fontSize:13,color:C.gold,fontFamily:"Playfair Display,serif",fontWeight:600}}>{wine.price_range}</div>
          )}
        ...
        {/* Lien Vivino */}
        <a href={wine.vivino_url||`https://www.vivino.com/search/wines?q=${encodeURIComponent(wine.name)}`}
          ...
          <span style={{fontSize:14}}>🍷</span> Voir sur Vivino
        </a>
      </div>
    );
  };
  ...
  const LoadingWine=({msg,elapsed,phase})=>{
    const steps=[
      {id:"generating",label:"Préparation par le Sommelier",done:phase==="fetching"||phase==="done"},
      {id:"fetching",  label:"Vivino en direct",             done:phase==="done"},
    ];
    return(
      <div style={{textAlign:"center",padding:"40px 20px"}}>
        ...
        <div style={{fontSize:12,color:C.muted,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginBottom:28}}>{elapsed}s…</div>
          ...
          {steps.map((s,i)=>{
            ...
            return(
              <div key={s.id} style={{display:"flex",alignItems:"center",gap:12}}>
                ...
                <span style={{fontSize:13,color:s.done?C.sauge:active?C.terra:C.muted,fontFamily:"Cormorant Garamond,serif",fontWeight:active||s.done?600:400}}>{s.label}</span>
              </div>
            );
          })}
        </div>
        ...
        <div style={{fontSize:11,color:C.muted,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic"}}>
          Données Vivino en direct — résultats en quelques secondes
        </div>
      </div>
    );
  };
  ...
  return(
    <div style={{flex:1,overflowY:"auto",padding:"14px 16px 100px"}}>
      ...
      <div style={{display:"flex",background:"rgba(139,90,60,.08)",border:`1px solid rgba(139,90,60,.15)`,borderRadius:14,padding:4,gap:4,marginBottom:18}}>
        {[["suggestions","🍷 Vivino"],["search","🔍 Recherche"]].map(([id,l])=>(
          <button key={id} onClick={()=>setMode(id)} style={{flex:1,padding:"10px",borderRadius:11,background:mode===id?`linear-gradient(135deg,${C.terra},${C.terraD})`:"transparent",color:mode===id?"#fff":C.subtext,fontSize:11,letterSpacing:".06em",fontFamily:"Cormorant Garamond,serif",transition:"all .25s"}}>{l}</button>
        ))}
      </div>
        ...
        <>
          {vivinoPhase==="idle"&&(
            <div style={{textAlign:"center",padding:"30px 20px"}}>
              ...
              <p style={{fontSize:13,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",lineHeight:1.7,marginBottom:8}}>
                {liked.length>0
                  ? `Basé sur vos ${liked.length} vins préférés, votre sommelier va consulter Vivino pour vous trouver les perles rares du moment.`
                  : "Votre sommelier va consulter Vivino pour vous proposer une sélection du jour."}
              </p>
              <p style={{fontSize:11,color:C.muted,fontFamily:"Cormorant Garamond,serif",marginBottom:28}}>⏱ Environ 30–60 secondes</p>
              <button onClick={loadVivinoSuggestions} style={{background:`linear-gradient(135deg,${C.terra},${C.terraD})`,color:"#fff",border:"none",padding:"16px 32px",borderRadius:50,fontSize:12,letterSpacing:".2em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif",fontWeight:700,boxShadow:`0 8px 28px rgba(200,80,58,.3)`}}>
                Consulter Vivino →
              </button>
            </div>
          )}
          ...
          {vivinoPhase==="error"&&(
            <div style={{textAlign:"center",padding:"30px 20px"}}>
              ...
              <div style={{fontSize:12,color:C.muted,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic",marginBottom:24,lineHeight:1.6}}>{vivinoError}</div>
              <button onClick={loadVivinoSuggestions} style={{background:`rgba(200,80,58,.12)`,border:`1px solid rgba(200,80,58,.35)`,borderRadius:22,padding:"11px 26px",color:C.terra,fontSize:13,fontFamily:"Playfair Display,serif"}}>Réessayer</button>
            </div>
          )}
          ...
          {vivinoPhase==="done"&&vivinoWines&&(
            <>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-end",marginBottom:16}}>
                <div>
                  <h2 style={{fontSize:22,fontFamily:"Playfair Display,serif",color:C.cream,fontWeight:400,marginBottom:4}}>Sélection Vivino</h2>
                  <p style={{fontSize:12,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic"}}>Données en direct · {vivinoWines.length} vins</p>
                </div>
                <button onClick={()=>{setVivinoPhase("idle");setVivinoWines(null);}} style={{flexShrink:0,display:"flex",alignItems:"center",gap:6,background:"rgba(184,134,42,.1)",border:`1px solid rgba(184,134,42,.3)`,borderRadius:22,padding:"8px 14px",color:C.gold,fontSize:11,fontFamily:"Cormorant Garamond,serif"}}>
                  ↺ Nouvelle
                </button>
              </div>
      ...
      {mode==="search"&&(
        <>
          <div style={{marginBottom:16}}>
            <h2 style={{fontSize:22,fontFamily:"Playfair Display,serif",color:C.cream,fontWeight:400,marginBottom:4}}>Recherche Vivino</h2>
            <p style={{fontSize:12,color:C.subtext,fontFamily:"Cormorant Garamond,serif",fontStyle:"italic"}}>Décrivez ce que vous cherchez, Vivino trouve.</p>
          </div>
          ...
          <div style={{display:"flex",gap:8,marginBottom:14}}>
            <input value={query} onChange={e=>setQuery(e.target.value)}
              ...
              style={{flex:1,background:C.bgCard,border:`1px solid rgba(139,90,60,.22)`,borderRadius:14,padding:"12px 15px",color:C.cream,fontSize:13,fontFamily:"Cormorant Garamond,serif",boxShadow:`0 2px 8px rgba(139,90,60,.06)`}}/>
          ...
          {searchPhase==="idle"&&!searchResults&&(
            <div style={{display:"flex",flexWrap:"wrap",gap:8,marginBottom:18}}>
              {["Bordeaux puissant","Blanc minéral","Champagne grower","Vin naturel","Malbec argentin","Riesling sec","Sangiovese","Orange wine"].map(s=>(
                <button key={s} onClick={()=>setQuery(s)} style={{background:"rgba(184,134,42,.08)",border:"1px solid rgba(184,134,42,.22)",borderRadius:20,padding:"7px 13px",fontSize:11,color:C.subtext,fontFamily:"Cormorant Garamond,serif"}}>{s}</button>
              ))}
            </div>
          )}
          ...
          {(searchPhase==="searching"||searchPhase==="vivino")&&(
            <LoadingWine
              msg={searchPhase==="searching"?"Claude prépare la recherche…":"Consultation Vivino…"}
              elapsed={searchElapsed}
              phase={searchPhase==="searching"?"generating":"fetching"}
            />
          )}
          ...
          {searchPhase==="error"&&(
            <div style={{background:`rgba(200,80,58,.07)`,border:`1px solid rgba(200,80,58,.22)`,borderRadius:14,padding:"13px 16px",textAlign:"center",marginBottom:14}}>
              <div style={{fontSize:12,color:C.terra,fontFamily:"Cormorant Garamond,serif",marginBottom:10}}>{searchError}</div>
              <button onClick={doSearch} style={{color:C.gold,textDecoration:"underline",fontSize:13,fontFamily:"Cormorant Garamond,serif",background:"none"}}>Réessayer</button>
            </div>
          )}
          ...
          {searchPhase==="done"&&searchResults&&(
            <>
              <div style={{display:"flex",justifyContent:"space-between",alignItems:"center",marginBottom:12}}>
                <div style={{fontSize:11,color:C.subtext,fontFamily:"Cormorant Garamond,serif"}}>{searchResults.length} résultat{searchResults.length>1?"s":""} Vivino</div>
...
export default function UnwinedApp() {
  ...
  const [screen,setScreen]=useState("cave");
  ...
  const TABS=[{id:"cave",label:"Ma Cave",icon:"🗄️"},{id:"add",label:"Ajouter",icon:"＋"},{id:"scan",label:"Scanner",icon:"📷"},{id:"decouvrir",label:"Découvrir",icon:"✦"}];
  ...
  return(
    <div style={{maxWidth:430,margin:"0 auto",minHeight:"100vh",background:C.bg,fontFamily:"Cormorant Garamond,Georgia,serif",display:"flex",flexDirection:"column",position:"relative"}}>
      ...
      {/* ── SCREENS ── */}
      {screen==="add"&&<AddWineForm onSave={addWine} onCancel={()=>setScreen(tab)}/>}
