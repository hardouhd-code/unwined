...
export default function UnwinedApp() {
      ...
      if(profile.cave){
        try{
          ...
          const normalized = Array.isArray(parsed)
            ? parsed.map((w) => {
                const rawRating = w?.rating;
                const rating =
                  rawRating === undefined || rawRating === null || rawRating === ""
                    ? null
                    : Math.max(0, Math.min(5, Number(rawRating)));
                return {
                  ...w,
                  tasted: !!w.tasted,
                  storage: w.storage ?? true,
                  rating: Number.isFinite(rating) ? rating : null,
                  quantity: w.quantity ?? 0,
                  lowStockThreshold: w.lowStockThreshold ?? 1,
                };
              })
            : [];
          setDb(normalized);
  ...
  return(
    <div className="app-shell" style={{fontFamily:"Manrope, sans-serif",display:"flex",flexDirection:"column"}}>
      <style dangerouslySetInnerHTML={{__html:GLOBAL_CSS}}/>

      {/* ── HEADER ÉDITORIAL ── */}
      {!["detail","add"].includes(screen)&&(
        <div className="android-glass" style={{padding:"28px 20px 0",flexShrink:0}}>
          <div style={{display:"flex",justifyContent:"space-between",alignItems:"flex-start",marginBottom:16}}>
            <div>
              ...
              <h1 style={{fontSize:30,fontFamily:"Noto Serif,serif",fontWeight:500,lineHeight:1.05,margin:0,color:C.gold,letterSpacing:".14em",textTransform:"uppercase"}}>
                La Selection
              </h1>
            </div>
      ...
      {!["detail","add"].includes(screen)&&(
        <div style={{position:"fixed",bottom:"calc(12px + env(safe-area-inset-bottom))",left:"50%",transform:"translateX(-50%)",width:"min(94vw,430px)",zIndex:100}}>
          <div className="android-glass" style={{display:"flex",borderRadius:18,padding:"6px",boxShadow:"0 16px 40px rgba(0,0,0,.35)"}}>
            {TABS.map(tab_item=>{
              ...
              return(
                <button key={tab_item.id} onClick={()=>{haptic(30);if(tab_item.id==="scan"){setScreen("scan");setTab("scan");}else{setTab(tab_item.id);setScreen(tab_item.id);}}}
                  style={{flex:1,position:"relative",background:isActive?"rgba(197,160,89,.18)":"none",border:"none",borderRadius:14,padding:"12px 4px 10px",display:"flex",flexDirection:"column",alignItems:"center",gap:4}}>
                  <span style={{fontSize:18,lineHeight:1}}>{tab_item.icon}</span>
                  <span style={{fontSize:10,letterSpacing:".06em",textTransform:"uppercase",color:isActive?C.gold:C.muted,fontFamily:"Manrope,sans-serif",lineHeight:1.1,fontWeight:isActive?700:600}}>{tab_item.label}</span>
                </button>
              );
            })}
          </div>
