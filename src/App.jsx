...
const WineInfoCard=({wine})=>{
  ...
  return(
    <div style={{background:"rgba(184,134,42,.07)",border:`1px solid rgba(184,134,42,.22)`,borderRadius:16,padding:"16px",marginBottom:14,animation:"fadeIn .4s ease"}}>
      ...
      <a href={`https://www.vivino.com/search/wines?q=${encodeURIComponent(info.search_query||q)}`} target="_blank" rel="noopener noreferrer" style={{display:"inline-flex",alignItems:"center",gap:6,background:`rgba(200,80,58,.12)`,border:`1px solid rgba(200,80,58,.3)`,borderRadius:20,padding:"7px 16px",textDecoration:"none",color:C.terra,fontSize:10,letterSpacing:".12em",textTransform:"uppercase",fontFamily:"Cormorant Garamond,serif"}}>🔍 Voir sur Vivino</a>
    </div>
  );
};
...
[{"name":"Château X","producer":"Domaine Y","type":"rouge","region":"Bordeaux","country":"France","year":2019,"rating":"4.2","ratings_count":"52000","price_range":"25-35€","grapes":"Merlot, Cabernet","description":"2 phrases arômes et style","search_query":"Château X 2019"}]`;
  ...
  return wines.map(w => ({
    ...w,
    type: w.type||"autre",
    vivino_url: `https://www.vivino.com/search/wines?q=${encodeURIComponent(w.search_query||`${w.name} ${w.year||""}`.trim())}`,
    image: null,
  }));
}
/* ═══════════════════════════════════════════════════════════════
