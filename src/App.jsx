...
const SmartScanner=({profile,onResult,onBack})=>{
  ...
  const scan=async()=>{
    ...
    try{
      // 1. Capturer la frame vidéo en base64
      ...
      // 2. Appel Claude Vision pour lire l'étiquette
      const messages=imageBase64
        ? [{role:"user",content:[
            {type:"image",source:{type:"base64",media_type:"image/jpeg",data:imageBase64}},
            {type:"text",text:`Tu es un expert en vins. Analyse cette étiquette de bouteille et réponds UNIQUEMENT en JSON strict :
{"name":"Nom complet du vin","domaine":"Producteur","grape":"Cépage","region":"Région, Pays","year":2019,"vivinoRating":4.2,"priceEst":"42 €","aeration":45,"pH":3.85,"tannins":8,"glycerol":8,"malo":true,"story":"3 phrases poétiques sur ce vin","anecdote":"1 fait surprenant sur ce domaine ou ce millésime","emoji":"🍷","vibe":"cocon"}
- Si l'étiquette n'est pas lisible, devine à partir de ce que tu vois
- vibe doit être un de : cocon, orage, secret, heure-bleue, evasion, patrimoine, impro, revolte
- vivinoRating entre 3.5 et 4.8, style poétique jamais technique`}
          ]}]
        : [{role:"user",content:`Invente un vin de qualité fictif mais réaliste. JSON strict :
{"name":"Vin imaginaire","domaine":"Domaine","grape":"Cépage","region":"Région, Pays","year":2020,"vivinoRating":4.1,"priceEst":"35 €","aeration":30,"pH":3.85,"tannins":8,"glycerol":8,"malo":true,"story":"Histoire poétique","anecdote":"Fait surprenant","emoji":"🍷","vibe":"cocon"}`}];
      ...
      const txt=d.content.map(b=>b.text||"").join("").replace(/```json|```/g,"").trim();
      const parsed=JSON.parse(txt);
      const tier=PRICE_TIERS[1];
      setWine({...parsed,
        id:`scan-${Date.now()}`,
        baseCompat:85,price:parsed.price||"€€",
        note:"",addedAt:new Date().toLocaleDateString("fr-FR"),openedAt:null
      });
    }catch(e){
      // Fallback WINE_BANK si API ou caméra indispo
      const vibes=["cocon","secret","orage","heure-bleue","evasion"];
      const styles=["epice","mineral","tanins","merlot","nouveau"];
      const budgets=["decouverte","heritage","exception"];
      const rv=vibes[Math.floor(Math.random()*vibes.length)];
      const rs=styles[Math.floor(Math.random()*styles.length)];
      const rb=budgets[Math.floor(Math.random()*budgets.length)];
      const fallback=pickFromBank(rv,rs,rb,Math.floor(Math.random()*3));
      setWine(fallback||{id:`scan-fb-${Date.now()}`,name:"Sélection du Sommelier",
        domaine:"Domaine à découvrir",grape:"Blend",region:"France",year:2020,
        vibe:"cocon",vivinoRating:4.1,priceEst:"35 €",aeration:30,
        story:"Ce vin attend de vous révéler ses secrets.",emoji:"🍷",
        baseCompat:82,price:"€€",note:"",
        addedAt:new Date().toLocaleDateString("fr-FR"),openedAt:null});
    }
    setPhase("result");
  };
