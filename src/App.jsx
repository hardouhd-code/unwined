...
const supa = {
        ...
        try {
          ...
          if(s.refresh_token) {
            try {
              ...
              // Only force logout on explicit auth errors, not network errors
          ...
          // No refresh token but keep user if we have their data
          if(s.user) return s;
          localStorage.removeItem("unwined_session");
          supa._token=null; supa._user=null;
        }
  ...
  async saveProfile(userId, name, cave) {
    ...
    try {
      ...
      const patched = patchRes.status !== 204 ? await patchRes.json().catch(()=>[]) : [];
      if(patchRes.ok && Array.isArray(patched) && patched.length > 0) {
        console.log("✓ PATCH ok");
        return;
      }
    } catch(e){ console.warn("PATCH attempt:", e.message); }

    // Step 2: INSERT new row
    const insertRes = await fetch(`${SUPA_URL}/rest/v1/profiles`, {
      method:"POST",
      headers:{
        "apikey": SUPA_KEY,
        "Authorization": `Bearer ${supa._token}`,
