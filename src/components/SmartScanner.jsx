...
{"topPick":{"name":"","producer":"","type":"rouge|blanc|rose|mousseux|autre","country":"","region":"","year":2020,"match":85,"why":"2 phrases poétiques","story":"3 phrases terroir","taste_profile":{"nose":"arômes au nez","palate":"saveurs en bouche","finish":"finale","body":"leger|moyen|ample","acidity":"faible|moyenne|vive","tannins":"souples|moyens|puissants"},"serving":{"temperature":"ex: 16-18°C","pairing":"accord mets-vin conseillé"},"emoji":"🍷","grapes":"","price_range":""},"boirSuggestions":[{"name":"Nom Vin 1","reason":"Pourquoi ce vin ressemble au scanné"},{"name":"Nom Vin 2","reason":"Une alternative accessible en Belgique"}]}`;
        ...
        try {
          ...
          const withConfidence = {
            ...parsedResult,
            topPick: {
              ...(parsedResult.topPick || {}),
              confidence: parsedResult?.topPick?.confidence ?? Math.max(55, Math.min(98, Number(parsedResult?.topPick?.match || 70))),
            }
          };
          setResult(withConfidence);
          setPhase("result");
        ...
        <div style={{ background: `linear-gradient(160deg,${typeLight(p.type)} 0%,transparent 40%)` }}>
          <div style={{ padding: "36px 20px 80px" }}>
            ...
            {(taste.nose || taste.palate || taste.finish || taste.body || taste.acidity || taste.tannins) && (
              <div style={{ background: "rgba(233,193,118,.08)", border: "1px solid rgba(233,193,118,.25)", borderRadius: 14, padding: "14px 16px", marginBottom: 14 }}>
                <div style={{ fontSize: 13, color: C.gold, letterSpacing: ".18em", textTransform: "uppercase", marginBottom: 8, fontFamily: "Cormorant Garamond,serif" }}>
                  Profil de goût
                </div>
                {taste.nose && <p style={{ fontSize: 13, color: C.subtext, marginBottom: 5 }}><strong>Nez:</strong> {taste.nose}</p>}
                {taste.palate && <p style={{ fontSize: 13, color: C.subtext, marginBottom: 5 }}><strong>Bouche:</strong> {taste.palate}</p>}
                {taste.finish && <p style={{ fontSize: 13, color: C.subtext, marginBottom: 5 }}><strong>Finale:</strong> {taste.finish}</p>}
                <div style={{ display: "flex", gap: 6, flexWrap: "wrap", marginTop: 6 }}>
                  {taste.body && <span style={{ fontSize: 11, color: C.muted, background: "rgba(48,40,34,.7)", borderRadius: 12, padding: "3px 8px" }}>Corps: {taste.body}</span>}
                  {taste.acidity && <span style={{ fontSize: 11, color: C.muted, background: "rgba(48,40,34,.7)", borderRadius: 12, padding: "3px 8px" }}>Acidité: {taste.acidity}</span>}
                  {taste.tannins && <span style={{ fontSize: 11, color: C.muted, background: "rgba(48,40,34,.7)", borderRadius: 12, padding: "3px 8px" }}>Tanins: {taste.tannins}</span>}
                </div>
                {(serving.temperature || serving.pairing) && (
                  <div style={{ marginTop: 10, borderTop: "1px solid rgba(233,193,118,.2)", paddingTop: 8 }}>
                    {serving.temperature && <p style={{ fontSize: 12, color: C.subtext, marginBottom: 4 }}><strong>Service:</strong> {serving.temperature}</p>}
                    {serving.pairing && <p style={{ fontSize: 12, color: C.subtext }}><strong>Accord:</strong> {serving.pairing}</p>}
                  </div>
                )}
              </div>
            )}
