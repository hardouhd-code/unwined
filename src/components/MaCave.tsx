...
const MaCave = () => {
  ...
  const stats = {
    ...
    avg: db.filter(w => w.tasted && w.rating !== null).length
      ? (db.filter(w => w.tasted && w.rating !== null).reduce((a, w) => a + w.rating, 0) / db.filter(w => w.tasted && w.rating !== null).length).toFixed(1)
      : "—"
  };
  ...
  const chartData = useMemo(() => {
    ...
    db.forEach(w => { counts[w.type] = (counts[w.type] || 0) + 1; });
  ...
  const changeQty = (wine: Wine, delta: number) => {
    const currentQty = Math.max(0, Number(wine.quantity ?? 0));
        ...
        return (
          <button key={wine.id} onClick={() => { haptic(30); onOpenWine(wine); }}
            ...
            <div className="flex-1 min-w-0">
              ...
              <div className="text-[13px] text-[var(--color-subtext)] font-['Manrope',sans-serif] mt-0.5 truncate">
                {[wine.year, wine.name && wine.name !== wine.producer ? wine.name : null].filter(Boolean).join(" · ")}
              </div>
            </div>
            <div className="flex items-center gap-2 ml-0.5" onClick={(e) => e.stopPropagation()}>
              ...
              <span className="min-w-[14px] text-center text-[22px] font-['Noto_Serif',serif] font-bold text-[var(--color-cream)]">
                {Math.max(0, Number(wine.quantity ?? 0))}
              </span>
              <button
                onClick={() => { haptic(20); changeQty(wine, +1); }}
                className="w-[30px] h-[30px] rounded-full border border-[#c5a05947] bg-transparent text-[var(--color-gold)] flex items-center justify-center text-lg leading-none"
  ...
  return (
    <div className="flex-1 overflow-y-auto px-4 pt-3.5 pb-[100px] bg-[var(--color-bg)]">
      ...
      <div className="flex items-center justify-between mb-3.5">
        ...
        <button
          onClick={() => { haptic(40); onAdd(); }}
          className="flex items-center gap-1.5 bg-gradient-to-br from-[var(--color-terra)] to-[var(--color-terra-dark)] text-white border-none px-4 py-2.5 rounded-full text-[11px] tracking-[.15em] uppercase font-['Cormorant_Garamond',serif] font-bold shadow-[0_6px_18px_rgba(200,80,58,.25)]"
        >
          + Ajouter
        </button>
      </div>
      ...
      <div className="grid grid-cols-4 gap-2 mb-3.5">
        {([[stats.total, "Vins", "var(--color-cream)"], [stats.inCave, t("filter_cave"), "var(--color-gold)"], [stats.tasted, t("filter_tasted"), "var(--color-terra)"], [stats.avg, "Moy.", "var(--color-sauge)"]] as [any, string, string][]).map(([v, l, col]) => (
          <div key={l} className="bg-[var(--color-bg-card)] border border-[var(--color-border-subtle)] rounded-2xl py-2.5 px-1.5 text-center shadow-[0_20px_40px_rgba(37,22,14,.2)]">
            <div className="text-[19px] font-['Playfair_Display',serif] font-semibold leading-none" style={{ color: col }}>{v}</div>
            <div className="text-[10px] text-[var(--color-muted-text)] tracking-[.1em] uppercase font-['Cormorant_Garamond',serif] mt-0.5">{l}</div>
          </div>
        ))}
      </div>
                ...
                <div onClick={(e) => e.stopPropagation()} className="flex justify-between items-center mt-1">
                  ...
                  <div className="flex gap-1.5">
                    {boirUrlByWineId.get(wine.id) ? (
                      <a href={boirUrlByWineId.get(wine.id)} target="_blank" rel="noreferrer"
      ...
      <div className="flex gap-1.5 mb-2 overflow-x-auto pb-1 no-scrollbar">
        <button onClick={() => setTypeFilter(null)}
          className={`rounded-full px-3 py-1.5 text-[10px] whitespace-nowrap font-['Manrope',sans-serif] ${!typeFilter ? "bg-[var(--color-gold)] border-[#e9c17666] text-[#412d00]" : "bg-[#302822] border-[rgba(197,160,89,.18)] text-[var(--color-subtext)]"}`}>
          Tous
        </button>
        {WINE_TYPES.map(t => (
          <button key={t.id} onClick={() => setTypeFilter(typeFilter === t.id ? null : t.id)}
            className={`rounded-full px-3 py-1.5 text-[10px] whitespace-nowrap font-['Manrope',sans-serif] ${typeFilter === t.id ? "bg-[#e9c1762e] border-[#e9c17673] text-[var(--color-gold)]" : "bg-[#302822] border-[rgba(197,160,89,.15)] text-[var(--color-subtext)]"}`}>
            {t.emoji} {t.label}
          </button>
        ))}
      </div>
      ...
      <div className="flex gap-1.5 mb-3.5 overflow-x-auto pb-1 no-scrollbar">
        {([["all", t("filter_all")], ["cave", t("filter_cave")], ["tasted", t("filter_tasted")], ["top", "⭐ Top"], ["avoid", "🚫 Éviter"]] as [string, string][]).map(([id, l]) => (
          <button key={id} onClick={() => setFilter(id)}
            className={`rounded-full px-3 py-1.5 text-[10px] whitespace-nowrap font-['Manrope',sans-serif] ${filter === id ? "bg-[#e9c1762e] text-[var(--color-gold)]" : "bg-[#302822] text-[var(--color-subtext)]"}`}>
            {l}
          </button>
        ))}
      </div>
      ...
      {/* ── LISTE ── */}
      {displayed.length === 0 ? (
        <div className="text-center py-10 text-[var(--color-muted-text)] font-['Cormorant_Garamond',serif] italic text-sm">Aucun vin dans cette catégorie.</div>
      ) : filter === "all" ? (
        <div>
          <div className="mb-2.5 mt-0.5">
            <button onClick={() => setShowCaveSection(v => !v)} className="w-full flex items-center justify-between bg-transparent border-none p-0 cursor-pointer">
          ...
          <div className="mb-2.5">
            <button onClick={() => setShowTastedSection(v => !v)} className="w-full flex items-center justify-between bg-transparent border-none p-0 cursor-pointer">
