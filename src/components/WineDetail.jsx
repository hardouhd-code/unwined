      ...
      <div style={{background:`linear-gradient(160deg,${tl} 0%,transparent 40%)`,minHeight:"100vh"}}>
        ...
        <div style={{padding:"0 20px 80px"}}>
          ...
          <div style={{background:C.bgCard,border:"1px solid rgba(139,90,60,.12)",borderRadius:18,padding:"16px 20px",marginBottom:14,display:"flex",alignItems:"center",justifyContent:"space-between",gap:16}}>
            <div>
              ...
              <div style={{display:"flex",alignItems:"center",gap:8,marginTop:8}}>
                <span style={{fontSize:11,color:C.muted,letterSpacing:".1em",textTransform:"uppercase"}}>Seuil stock bas</span>
                <button onClick={()=>onUpdateWine&&onUpdateWine(wine.id,{lowStockThreshold:Math.max(0,lowThreshold-1)})} style={{width:22,height:22,borderRadius:"50%",background:"rgba(139,90,60,.1)",border:"1px solid rgba(139,90,60,.2)",color:C.cream,fontSize:14}}>−</button>
                <span style={{fontSize:14,color:C.cream,minWidth:14,textAlign:"center"}}>{lowThreshold}</span>
                <button onClick={()=>onUpdateWine&&onUpdateWine(wine.id,{lowStockThreshold:lowThreshold+1})} style={{width:22,height:22,borderRadius:"50%",background:"rgba(139,90,60,.1)",border:"1px solid rgba(139,90,60,.2)",color:C.cream,fontSize:14}}>+</button>
              </div>
              {isLow && (
                <div style={{marginTop:8,fontSize:12,color:"#ffb68c"}}>
          ...
          <a
            ...
            target="_blank"
            rel="noreferrer"
            style={{display:"inline-flex",alignItems:"center",gap:8,marginBottom:14,textDecoration:"none",background:"rgba(233,193,118,.12)",border:"1px solid rgba(233,193,118,.35)",borderRadius:20,padding:"8px 12px",color:C.gold,fontSize:12}}
          >
            ↗ Racheter sur Boir
          </a>
