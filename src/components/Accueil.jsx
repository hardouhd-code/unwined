...
const Accueil = ({ db, userName, onSetName, onAdd, onOpenWine }) => {
  ...
  // Recherche des vins réels sur boir.be via notre proxy Vercel
  const searchBoir = async (query) => {
    try {
      const r = await fetch(`/api/boir-search?q=${encodeURIComponent(query)}`);
  ...
  const loadWines = async (attempt = 0) => {
    ...
    try {
      ...
      // ── ÉTAPE 1 : Claude choisit 3 mots-clés de recherche adaptés aux goûts ──
...
[Session ${_seed}] Pour ${userName || "cet utilisateur"}, génère 3 termes de recherche simples pour trouver des vins sur un site e-commerce belge.
${liked ? `Goûts appréciés : ${liked}.` : "Pas encore de préférences — propose des classiques."}
${disliked ? `À éviter : ${disliked}.` : ""}
${last5 ? `Derniers vins : ${last5}.` : ""}
1 terme par gamme : budget(10-30€), milieu(30-75€), prestige(75€+).
Chaque terme = 1-3 mots simples comme "bordeaux rouge", "chablis", "barolo", "sancerre", "rioja".
FORMAT : [{"query":"bordeaux","budget":"budget"},{"query":"chablis premier cru","budget":"milieu"},{"query":"barolo","budget":"prestige"}]`;

      const queryTxt = await callClaude([{ role: "user", content: queryPrompt }], 300);
      const queries  = safeJson(queryTxt, []);
      const qList    = Array.isArray(queries) ? queries.slice(0, 3) : [
        { query: "bordeaux rouge", budget: "budget" },
        { query: "bourgogne blanc", budget: "milieu" },
        { query: "barolo", budget: "prestige" },
      ];

      // ── ÉTAPE 2 : Chercher ces termes sur boir.be (vins RÉELS) ──
      const boirMatches = await Promise.all(qList.map(async ({ query, budget }) => {
        const results = await searchBoir(query);
        const match   = results.find(r => r.available) || results[0] || null;
        return match ? { ...match, budget } : null;
      }));

      const foundWines = boirMatches.filter(Boolean);
