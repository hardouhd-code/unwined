...
const Accueil = ({ db, userName, onSetName, onAdd, onOpenWine }) => {
  ...
  // Recherche un vin sur boir.be via notre proxy Vercel
...
[{"name":"Château X","producer":"Domaine Y","type":"rouge","region":"Bordeaux","country":"France","year":2020,"price_range":"18-24€","grapes":"Merlot, Cabernet","description":"2 phrases arômes et style","why":"Raison personnalisée","search_query":"Chateau X 2020","budget":"budget"},{"name":"...","producer":"...","type":"blanc","region":"...","country":"...","year":2021,"price_range":"45-55€","grapes":"...","description":"...","why":"...","search_query":"...","budget":"milieu"},{"name":"...","producer":"...","type":"rouge","region":"...","country":"...","year":2019,"price_range":"90-120€","grapes":"...","description":"...","why":"...","search_query":"...","budget":"prestige"}]`;
      ...
      // Vérification boir.be en parallèle pour chaque vin
      const enriched = await Promise.all(list.map(async (wine) => {
        const q = wine.search_query || `${wine.producer} ${wine.name}`;
        const boirResults = await searchBoir(q);
        const match = boirResults.find(r => r.available) || boirResults[0] || null;
        return {
          ...wine,
          boir_url:      match ? match.url   : null,
          boir_price:    match ? match.price : null,
          boir_found:    !!match,
          boir_title:    match ? match.title : null,
        };
      }));
      ...
      {phase === "done" && wines && wines.map((wine, i) => {
        ...
        return (
          <div
            ...
            <a
              href={wine.boir_found
                ? wine.boir_url
                : `https://boir.be/fr/recherche?search_query=${encodeURIComponent((wine.search_query || wine.name).replace(/\s+/g, "+"))}`}
              target="_blank" rel="noopener noreferrer"
              style={{
                display: "flex", alignItems: "center", justifyContent: "center", gap: 8,
                background: wine.boir_found ? `linear-gradient(135deg, rgba(184,134,42,.12), rgba(184,134,42,.06))` : "transparent",
                border: wine.boir_found ? `1px solid rgba(184,134,42,.4)` : `1px solid rgba(200,80,58,.3)`,
            ...
            >
              {wine.boir_found ? (
                ...
                <span style={{ color: C.terra }}>🍷 Rechercher sur Boir.be</span>
              )}
            </a>
          </div>
        );
