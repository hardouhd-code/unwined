  ...
  return (
    <div style={{ background: THEME.card, border: `1px solid ${THEME.border}`, borderRadius: '16px', padding: '16px', display: 'flex', gap: 12, marginBottom: '12px', boxShadow: '0 4px 12px rgba(139,90,60,0.08)' }}>
      ...
      <div style={{ flex: 1, minWidth: 0 }}>
        <div style={{ fontSize: '10px', color: THEME.accent, fontWeight: 'bold', letterSpacing: '0.5px' }}>{r}</div>
...
export function Decouvrir() {
  ...
  const results = useMemo(() => {
    try {
      ...
      // 1. Recherche par texte (Barre de recherche)
      if (query.trim().length >= 2) {
        return searchBoirLocal(query) || [];
      }

      // 2. Si aucun pays sélectionné, on ne montre rien
      ...
      return catalog.filter(w => {
        ...
        // FILTRE HYBRIDE : On cherche dans le champ région OU dans le titre
        const rName = norm(w.r || w.region || '');
        const tName = norm(w.t || w.title || '');
        const searchVal = norm(targetRegion);
        
        return rName === searchVal || tName.includes(searchVal);
      });
    } catch (e) {
      return [];
    }
