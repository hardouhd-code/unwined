...
function WineCard({ wine, isFavorite, onToggleFavorite, stockQty }) {
  if (!wine) return null;
    ...
    <div style={{ padding: '0 20px 140px', minHeight: '100vh', background: THEME.bg }}>
      ...
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', marginBottom: 15, scrollbarWidth: 'none' }}>
        {['Tous', 'Rouge', 'Blanc', 'Rosé', 'Bulles'].map(c => (
          <button key={c} onClick={() => setTypeFilter(c)} style={{ flexShrink: 0, padding: '10px 18px', borderRadius: 25, background: typeFilter === c ? THEME.accent : THEME.surfaceHigh, color: typeFilter === c ? '#412d00' : THEME.text, border: 'none', fontWeight: '700', fontSize: 11, letterSpacing: '.12em', textTransform: 'uppercase' }}>
            {c}
          </button>
        ))}
      </div>
      <div style={{ display: 'flex', gap: 8, overflowX: 'auto', marginBottom: 10, scrollbarWidth: 'none' }}>
        {PRICE_FILTERS.map((p) => (
          <button key={p} onClick={() => setPriceFilter(p)} style={{ flexShrink: 0, padding: '8px 14px', borderRadius: 20, background: priceFilter === p ? THEME.accent : THEME.surfaceHigh, color: priceFilter === p ? '#412d00' : THEME.text, border: 'none', fontWeight: 700, fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase' }}>
            {p}
          </button>
        ))}
        <button onClick={() => setOnlyFavorites((v) => !v)} style={{ flexShrink: 0, padding: '8px 14px', borderRadius: 20, background: onlyFavorites ? '#ffb68c' : THEME.surfaceHigh, color: onlyFavorites ? '#412d00' : THEME.text, border: 'none', fontWeight: 700, fontSize: 10, letterSpacing: '.08em', textTransform: 'uppercase' }}>
