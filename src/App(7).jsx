    ...
    <div style={{minHeight:"100vh",background:C.bg,overflowY:"auto"}}>
          ...
          <div style={{background:C.bgCard,border:`1px solid ${tc}33`,borderRadius:22,padding:"22px",marginBottom:14,boxShadow:`0 4px 24px rgba(139,90,60,.1)`}}>
            ...
            {(wine.quantity!=null)&&(
              <div style={{marginTop:16,display:"flex",alignItems:"center",justifyContent:"center",gap:14}}>
                <div style={{fontSize:12,color:C.muted,fontFamily:"Cormorant Garamond,serif",letterSpacing:".15em",textTransform:"uppercase"}}>En cave</div>
                <div style={{display:"flex",alignItems:"center",gap:10,background:"rgba(139,90,60,.06)",border:"1px solid rgba(139,90,60,.15)",borderRadius:50,padding:"6px 16px"}}>
                  <button onClick={()=>onUpdateWine&&onUpdateWine(wine.id,{quantity:Math.max(0,(wine.quantity||0)-1)})} style={{width:28,height:28,borderRadius:"50%",background:"rgba(139,90,60,.12)",border:"none",color:C.cream,fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:300}}>−</button>
                  <span style={{fontSize:22,fontFamily:"Playfair Display,serif",color:C.cream,minWidth:28,textAlign:"center"}}>{wine.quantity||0}</span>
                  <button onClick={()=>onUpdateWine&&onUpdateWine(wine.id,{quantity:(wine.quantity||0)+1})} style={{width:28,height:28,borderRadius:"50%",background:`linear-gradient(135deg,${C.terra},${C.terraD})`,border:"none",color:"#fff",fontSize:18,display:"flex",alignItems:"center",justifyContent:"center",fontWeight:300}}>＋</button>
                </div>
                <div style={{fontSize:12,color:C.muted,fontFamily:"Cormorant Garamond,serif"}}>bouteille{(wine.quantity||0)>1?"s":""}</div>
              </div>
            )}
          </div>
      ...
      {onDelete&&(
        <div style={{textAlign:"center",padding:"20px 0 60px"}}>
          <button onClick={()=>{if(window.confirm("Supprimer ce vin de votre cave ?"))onDelete(wine.id);}} style={{background:"none",border:"1px solid rgba(200,80,58,.3)",borderRadius:30,padding:"10px 24px",color:C.terra,fontSize:13,fontFamily:"Cormorant Garamond,serif",letterSpacing:".1em"}}>
            🗑 Supprimer de ma cave
          </button>
        </div>
      )}
    </div>
